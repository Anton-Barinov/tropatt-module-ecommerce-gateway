...
* **Страна доставки** — канонично `delivery_address.country_code` (ISO 3166-1 alpha-2, приводится к
  верхнему регистру). Прежние коннекторы шлют ключ `country`: он по-прежнему принимается и
  нормализуется в `country_code`, но только если это реальный ISO-код. Название страны
  (`shipping_country` OpenCart 3.0/4.0) кодом не считается и в `country_code` не попадает —
  коннектор обязан отдать `shipping_iso_code_2`/`country_code`, а читаемое название кладётся
  в `custom_fields`.
